...
const SeoEditor = ({seoData}: {seoData: TSEOData[]}) => {
  ...
  const handlePathChange = (selectedPath: string) => {
    const selectedContent = seoData.find(
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    setIsLoading(true);
    const filter = seoData.filter((data, i) => {
      return data.page !== form.watch("page");
    });

    const valuesToSendOnKv = [...filter, values];
    axios
      .put(`${evarConts.cloudflareKvUrl}/api/seo`, valuesToSendOnKv)
      .then((res) => {
